...
public class Handler2Players extends Handler {
    volatile private Snake Player2snake;
    volatile private Snake Player1snake;


    private int Player2DotEatenCount;
    private int fruitEatenCount;

    private int player2SkipMoveCount = 61;
    private int player1SkipMoveCount = 61;

    private boolean isPlayer2Dead;

    private GameGUI gameGUI;

    private int Player2X = 920;
    private int Player2Y = 540;
    private int Player2stepX;
    private int Player2stepY;
   ...
   private MyFont myFont = new MyFont();
    ...
    void imageInitializer() {
        ...
        try {
            ...
            headImagePlayer2 = headImageLeft2;
        ...
        for (Fruit fruit : getFruits()) {
            if (fruit.getCoordinates() != null) {
                ...
                if (fruit.getCoordinates() != null && fruitRectangle.intersects(snake1Rectangle)) {
                    ...
                    fruit.makeFruit();
                    fruitEatenCount++;
                    if (fruitEatenCount != 0 && fruitEatenCount % 10 == 0 && getPlayer1Speed() < 3) {
                        music.speedUpPlay();
                        setPlayer1Speed(3);
                        System.out.println(getPlayer1Speed());
                        speedControl();
                        fruitEatenCount = 0;
        ...
        while (!getIsEnd()) {
            try {
                ...
                if (player1SkipMoveCount <= 60){
                    ...
                    birdsLabelPlayer1.setVisible(false);
                    Player1snake.moveBody(getPlayer1X(), getPlayer1Y()); //chek it! Бывает NullPointException!!
                }
